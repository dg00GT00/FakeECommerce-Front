...
import {
	...
	FormState,
	UserFormButtonProps,
} from "../UserFormsTypes/UserFormsTypes";
...
type SignupType = Record<
	"email" | "password" | "generic",
	{ fieldValue: string }
>;
type LoginType = Record<"generic" | "email", { fieldValue: string }>;

export const UserActionButton: React.FunctionComponent<UserFormButtonProps> = (
	props
) => {
	const [errorSnack, setErrorSnack] = useSnackMessageError();
	const { registerUser, userLogin, userAddress } = React.useContext(
		AuthContext
	);
	const { goBack, push } = useHistory();

	const promiseError = React.useCallback(
		(formId: FormId, statusCode: number) => {
