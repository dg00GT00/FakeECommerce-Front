...
const snackbarStyle = makeStyles<Theme, {color?: string}>((theme) => {
    return createStyles({
        overrideColor: {
...
type SnackbarMessageProps = MessageStateProps & { key: number, snackOpen: boolean};
...
export const useUserSnackbar = (): [JSX.Element | null, (messageProps: MessageStateProps) => void] => {
    const [snackMessage, dispatchSnackMessage] = React.useReducer(messageReducer, initialMessageState);
    const style = snackbarStyle({color: snackMessage?.color});
