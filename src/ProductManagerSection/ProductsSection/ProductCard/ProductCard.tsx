...
type CardHighlightState = Partial<
	ReturnType<typeof useCardHighlightStyle> & {
...
type CardDispatchActions =
	| {
			type: "SELECT_CARD";
			productId: number;
			selectedState: CardHighlightState;
	  }
	| { type: "UNSELECT_CARD"; productId: number }
	| { type: "PREVIOUS_STATE" };

type CardHighlightReducer = React.Reducer<
	CardHighlightState,
	CardDispatchActions
>;
		...
		<div className={cardState.outerContainer}>
			<div className={cardState.innerContainer}>
				<Card className={styles.card}>
					...
					<CardActions
						className={[cardState.innerCard, styles.card_actions].join(" ")}
					>
						<div className={styles.price}>{`$ ${props.price}`}</div>
						<ButtonBase
							className={styles.cart_arrow_down_button}
							onClick={toggleProductSelection}
						>
							<CartArrowDown
								className={[cardState.innerCard, styles.cart_arrow_down].join(
									" "
								)}
							/>
						</ButtonBase>
